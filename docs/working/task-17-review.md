...
**12종 → 17종 → 16종**. 이유:
- 한국 시장 조사 결과 핵심 카테고리 누락 다수 발견 (12 → 17)
- 작업지시자 결정 (2026-04-28): redGinseng(홍삼) 제거 → 16종 최종
...
| ~~redGinseng 신규~~ → 제거 | 작업지시자 결정으로 본 카탈로그 범위 밖 (V1.1 검토) |
...
| 6 | vitaminB | 비타민 B | matte oblong | 핑크 레드 | 세로 score | ✅ |
| 7 | milkThistle | 밀크씨슬 | 캡슐 | 올리브 황녹 | 단색 캡슐 | ⏳ |
| 8 | glucosamine | 글루코사민 | 큰 matte oblong | 라이트 베이지 | 가로 score | ⏳ |
...
| 파일 | 변경 내용 |
|---|---|
| `docs/plans/task_W2_17_impl.md` | 12 → 16종 확장 + 시장조사 기반 결정 박제 + 17개 프롬프트 + 시각 변별 시스템 재정립 |
| `docs/adr/0007-server-catalog-as-source-of-truth.md` | 12 → 16종 확장 amendment 박제 |
| `scripts/category-spec.json` | 12 → 16 row, `_form` 필드 추가 (재질/형태 그룹 명시) |
| `scripts/imageset-categories.sh` | 16종 인자 + 주석 갱신 |
| `ios/PillPouch/Resources/category-seed.json` | 12 → 16 row, displayOrder 시장 점유율 기반 재정렬 |
| `docs/working/category-prompts.md` (NEW) | 11개 신규 프롬프트 박제 (운동 후 작업지시자가 바로 사용) |
| `docs/working/task-17-review.md` (NEW) | 본 문서 |

`design/categories/raw/`, `ios/PillPouch/Assets.xcassets/Categories/` 폴더는 변경 없음 — 작업지시자가 10장 도착시킨 후 Claude가 일괄 변환.
...
### Step 1: 10장 raw PNG 생성 (1~2시간)

`docs/working/category-prompts.md` 열어서 권장 순서대로:

1. **probiotics** (구매율 1위, 캡슐 형태 첫 시도)
2. **calcium** (사이즈 baseline)
3. **glucosamine** (가로 score 첫 시도)
4. **coq10** (omega3 변별 검증)
5. **lutein** (omega3 재질 변별)
6. **collagen** (probiotics 핑크 변별)
7. **iron** (다크 baseline)
8. **milkThistle** (iron 변별 — 갈색 vs 녹 vs 캡슐)
9. **magnesium** (그레이 분리)
10. **zinc** (그레이 그룹 마지막)
...
10장 모두 저장하시고 "10장 다 받음" 또는 "calcium만 받음" 등 알려주시면 Claude가 `./scripts/imageset-categories.sh` 실행해서 imageset 일괄 생성.
...
**작업지시자 운동 시간 동안 Claude가 시장조사 + 16종 확장 + 11개 프롬프트 정제 + 시각 변별 시스템 재정립 + 모든 인프라 문서 갱신 완료**. 운동 후엔 `docs/working/category-prompts.md` 보고 10장 raw 생성만 하시면 16종 시드 완성.
