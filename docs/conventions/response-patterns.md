...
### 시스템 알림에 짧게 응답할 때 — 미해결 결정 환기

`task-notification`, `stream ended`, hook 알림 같은 **시스템 자동 메시지**에 짧게 응답할 때, **직전에 사용자 결정 요청이 미해결 상태**면 그 요청을 다시 환기시킨다.

- ❌ "(대기 중)" 한 줄로만 끝내기 — 직전 결정 요청이 화면에서 밀려 사용자가 못 봄
- ✅ "(시스템 알림 처리됨. 직전 머지 승인 요청은 여전히 대기 중 → \`gh pr merge N --squash\`)" 식으로 한 줄 추가

이유: 사용자는 마지막 메시지만 보는 경우가 많음. 시스템 알림이 결정 요청을 가리면 무의미한 왕복("머지승인요청 오는거야?") 발생.

판단: 직전 응답이 사용자에게 결정을 요청한 채로 끝났는가? Yes면 매 시스템 알림 응답에 1줄로 환기.
...
M/L PR을 생성한 직후, **연결된 이슈와 동일한 라벨·마일스톤을 PR 객체에도 부착**한다.

### 왜 필요한가

- GitHub은 "Closes #N"이 있어도 PR 자체에 milestone/label을 자동 복사하지 않음
- PR 목록 화면(`/pulls`)에서 마일스톤·라벨로 필터링/그룹핑이 안 되어 주차별 진척이 안 보임
- 이슈는 닫히면 사라져서, 머지된 PR만 봐도 어느 주차/영역인지 식별돼야 함

### 절차

PR 생성 직후 (또는 머지 전 확인 시):

```bash
# 이슈에서 라벨/마일스톤 확인
gh issue view <ISSUE_NUM> --json labels,milestone

# PR에 동일하게 부착
gh pr edit <PR_NUM> --milestone "W{N}" --add-label "size:M,area:*,type:*"
```

### 체크리스트

- [ ] 이슈에 부여된 모든 라벨(size/area/type)이 PR에도 붙어 있는가
- [ ] 이슈의 마일스톤(W{N})이 PR에도 동일하게 부여됐는가
- [ ] 마지막 이슈가 닫힌 마일스톤이 있다면 마일스톤도 close 처리할지 작업지시자에 환기

### 사례 박제

- **2026-04-29 #12/#13/#20/#21/#22**: 머지 시점에 PR 메타 누락 → PR 목록에서 W1/W2 작업이 어느 주차인지 식별 불가 → 사용자가 스크린샷으로 지적. 이후 백필 + 본 룰 박제.
